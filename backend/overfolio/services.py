...
def get_all_rows(doc_name):
    ...
    for sheet in sh.worksheets():
        ...
        if raw:
            headers = raw[0]
            rows = raw[1:]
            all_data[sheet.title] = [dict(zip(headers, row)) for row in rows]
    return all_data
...
#def get_all_sheets_data(doc_name):
#    client = settings.GSPREAD_CLIENT
#    sh = client.open(doc_name)
#
#    data = {}
...
#        values = worksheet.get_all_values()  # list of lists
#        data[name] = values
#
#    return data
